"""
fetch_orbitrap_pos_hits.py
---------------------------
Fetch ALL reference + annotation library hits from MassWiki for the
Orbitrap HILIC posESI dataset (13,608 spectra).

Adapted from fetch_orbitrap_hits.py (negESI).
Input:  data/Orbitrap_HILIC_posESI_curated_041326.csv
Output: data/library_hits/orbitrap_hilic_pos_masswiki_hits.csv
Errors: data/library_hits/orbitrap_hilic_pos_masswiki_errors.csv

Usage:
    python fetch_orbitrap_pos_hits.py <MASSWIKI_TOKEN>
    # or set MASSWIKI_TOKEN env var
"""
import json
import os
import sys
...
SPREADSHEET = f'{ROOT}/data/Orbitrap_HILIC_posESI_curated_041326.csv'
OUT_HITS = f'{ROOT}/data/library_hits/orbitrap_hilic_pos_masswiki_hits.csv'
OUT_ERRORS = f'{ROOT}/data/library_hits/orbitrap_hilic_pos_masswiki_errors.csv'
CACHE_FILE = f'{ROOT}/data/library_hits/orbitrap_pos_fetch_cache.json'
...
if not TOKEN:
    print('ERROR: provide MassWiki token via argv or MASSWIKI_TOKEN env var')
    sys.exit(1)

# ── 1. Load wiki_ids ─────────────────────────────────────────────────────
print('Loading Orbitrap posESI CSV...')
df = pd.read_csv(SPREADSHEET, usecols=['wiki_id'], low_memory=False)
...
n_with = sum(1 for v in cache.values() if v and (v.get("ref") or v.get("anno")))
n_without = sum(1 for v in cache.values() if not v or (not v.get("ref") and not v.get("anno")))
print(f'\nSummary: {n_with} spectra with hits, {n_without} with no hits')
